...
class VueStoreFront(Router):
    __name__ = 'web_shop_vue_storefront'
    ...
    def __setup__(cls):
        ...
        cls.__routes__.update({
                'route': Route(
                    Rule('<shop>/<target>/<action>', methods={'POST', 'GET'}),
                    Rule('/<database_name>/web_shop_vue_storefront/'
                        '<shop>/<target>/<action>', methods={'POST', 'GET'},
                        redirect_to='<shop>/<target>/<action>'),
                    Rule('<shop>/<target>/<action>/<sku>', methods={'GET'}),
                    Rule('/<database_name>/web_shop_vue_storefront/'
                        '<shop>/<target>/<action>/<sku>', methods={'GET'},
                        redirect_to='<shop>/<target>/<action>/<sku>'),
                    decorators=[
                        with_pool,
    ...
    @classmethod
    def route(cls, request, pool, shop, target, action, sku=None):
