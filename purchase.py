...
class Purchase(ModelWorkflow, OSV):
    ...
    invoices_duplicated = fields.Many2Many('account.invoice',
            'purchase_invoice_duplicated_rel', 'purchase', 'invoice',
    ...
    def get_invoice_exception(self, cursor, user, purchases, context=None):
        ...
        for purchase in purchases:
            ...
            skip_ids.update(x.id for x in purchase.invoices_duplicated)
            for invoice in purchase.invoices:
...
class PurchaseLine(OSV):
    ...
    moves_duplicated = fields.Many2Many('stock.move',
            'purchase_line_moves_duplicated_rel', 'purchase_line', 'move',
    ...
    def get_invoice_line(self, cursor, user, line, context=None):
        ...
        if line.purchase.invoice_method == 'order':
            ...
            if line.purchase.invoices_ignored:
                skip_ids = set(
                    l.id for l in line.purchase.invoices_duplicated.lines)
            else:
                skip_ids = ()
    ...
    def get_purchase_exception_state(self, cursor, user, ids, name, arg,
        ...
        for move in self.browse(cursor, user, ids, context=context):
            if not move.purchase_line:
                continue
            if move.id in (x.id for x in move.purchase_line.moves_duplicated):
...
class Invoice(OSV):
    ...
    def __init__(self):
        ...
        self._error_messages.update({
                ...
                'reset_invoice': 'You cannot reset to draft an invoice '\
                    'generated by a purchase.',
            })
